<#
.SYNOPSIS
    Retrieves the latest quotes data for stocks from the Alpaca API.

.DESCRIPTION
    This function retrieves the latest quotes data for stocks from the Alpaca API based on the specified symbols, feed, and currency.
...
.EXAMPLE
    Get-AlpacaStockLatestQuotes -Symbols @("AAPL", "GOOGL", "MSFT") -Feed "IEX" -Currency "USD"
    Retrieves the latest quotes data for AAPL, GOOGL, and MSFT stocks with feed "IEX" and currency "USD".
#>

Function Get-AlpacaStockLatestQuotes {
    [CmdletBinding()]
    Param (
