<#
.SYNOPSIS
    Retrieves the latest bars data for stocks from the Alpaca API.

.DESCRIPTION
    This function retrieves the latest bars data for stocks from the Alpaca API based on the specified symbols, feed, and currency.
...
.EXAMPLE
    Get-AlpacaStockLatestBars -Symbols @("AAPL", "GOOGL", "MSFT") -Feed "IEX" -Currency "USD"
    Retrieves the latest bars data for AAPL, GOOGL, and MSFT stocks with feed "IEX" and currency "USD".
#>

Function Get-AlpacaStockLatestBars {
    [CmdletBinding()]
    Param (
